...
assert(n_types == 2, 'This function expects exactly 2 types (low/high energy-intensive).');
assert(abs(sum(lambda_type) - 1) < 1e-10, 'lambda_type must sum to 1.');
assert(all(gamma_type > 0), 'gamma_type entries must be positive.');
...
for s = 1:n_types
    ...
    assert(eta_s > 0, 'eta_s must be positive for type %d.', s);

    for i = 1:n_theta
        th = theta_grid(i);

        y_policy(i,s) = (A_type(s) * th)^(1/eta_s) ...
                      * (alpha/W)^(alpha/eta_s) ...
        ...
        pi_policy(i,s) = y_policy(i,s) - W*n_policy(i,s) - Pe*e_policy(i,s);
    end
    ...
    if it == maxit_vfi
        warning('VFI did not converge for type %d.', s);
    end
...
mu = repmat((lambda_type ./ n_theta), n_theta, 1);
mu = mu / sum(mu(:));
...
if it_mu == maxit_mu
    warning('Distribution iteration did not converge.');
end
...
exit_rate_agg = max(0, share_type' * exit_rate_type);
...
out.e_agg             = M_total * e_ave_total;
out.M_total           = M_total;
out.M_type            = M_type;
out.output_share_type = Y_type / sum(Y_type);
out.labor_share_type  = N_type / sum(N_type);
out.energy_share_type = E_type / sum(E_type);
out.vE_type           = vE_type;
out.share_type        = share_type;

out.V_type            = V_type;
out.exit_policy       = exit_policy;
out.mu                = mu;
out.mu_operate        = mu_survive;
out.theta_grid        = theta_grid;
out.Q_incumbent       = Q_incumbent;
out.Q_entrant         = Q_entrant;
out.y_policy          = y_policy;
out.n_policy          = n_policy;
out.e_policy          = e_policy;
out.pi_policy         = pi_policy;
out.y_ave_type        = y_ave_type;
out.n_ave_type        = n_ave_type;
out.e_ave_type        = e_ave_type;
out.y_agg             = M_total * y_ave_total;
out.n_agg             = M_total * n_ave_total;
out.Y_type            = Y_type;
out.N_type            = N_type;
out.E_type            = E_type;
out.M_entrants_total  = M_total * exit_rate_agg;
out.M_entrants_type   = (M_total * exit_rate_agg) * lambda_type';
end
